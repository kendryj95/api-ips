...
const email        = require('../../enviroments/notifications/email')
const sms          = require('../../enviroments/notifications/sms')
		...
		if (payment.state == 'approved') {
			...
			saveOnDatabase(payment, paymentId).then(data => {
				...
				email.new({
					...
					console.log(`MENSAJE ENVIADO: Message ${info.messageId} sent: ${info.response}`)
				}).catch(err => console.log('ERROR', err))

				// PROCESAR SMS
				sms.new({
					phone: data.client.phone,
					message: `Nuevo pago id ${data.idCompra} procesado satisfactoriamente en Insignia Payment Solutions.`
				}).then(res => {
					console.log('SMS PROCESADO', res)
				}).catch(err => console.log('ERROR EN PROCESO SMS', err))

/*				notification.new(email, sms).then(response => {
					console.log(`EMAIL ENVIADO: Message ${response.email.messageId} sent: ${response.email.response}`)
					console.log('SMS PROCESADO', response.sms)
				}).catch(err => console.log('NO SE PUDO ENVIAR LA NOTIFICACION', err)) */
